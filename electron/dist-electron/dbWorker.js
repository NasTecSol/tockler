...
const defaultHRIntegrationState = {
  username: "",
  authToken: null,
...
async function fetchHRIntegrationState() {
  ...
  try {
    ...
    logger$1.error("Error parsing HR_INTEGRATION_STATE:", e);
    return defaultHRIntegrationState;
  }
}
...
async function saveHRIntegrationState(state) {
...
async function findTrackItemsInRange(from, to) {
  return db.select().from(trackItems).where(and(lte(trackItems.beginDate, to), gte(trackItems.endDate, from))).orderBy(asc(trackItems.beginDate));
}
